...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #1a1a2e; margin: 0; padding: 20px;">
  ...
  /**
   * Invia email con codice OTP per autenticazione a due fattori
   * Template professionale con design coerente al resto dell'applicazione
   *
   * @param to - Email destinatario
   * @param code - Codice OTP a 6 cifre
   * @returns true se invio riuscito, false altrimenti
   */
  async sendOtpEmail(to: string, code: string): Promise<boolean> {
    const subject = 'Il tuo codice di verifica - Time Report';
    const html = `
<!DOCTYPE html>
<html>
<head>
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #1a1a2e; margin: 0; padding: 20px;">
  <div style="max-width: 600px; margin: 0 auto; background-color: #16213e; border-radius: 12px; overflow: hidden;">
    ...
    <div style="padding: 40px 30px;">
      <h2 style="color: #fff; margin: 0 0 20px;">Codice di Verifica</h2>

      <p style="color: #9ca3af; line-height: 1.6; margin: 0 0 20px;">
        Hai richiesto un codice di verifica per accedere a Time Report.
        Inserisci il seguente codice per completare l'autenticazione:
      </p>

      <div style="text-align: center; margin: 30px 0;">
        <div style="display: inline-block; background-color: #1a1a2e; padding: 20px 40px; border-radius: 8px; border: 2px solid #f59e0b;">
          <span style="font-size: 32px; font-weight: bold; color: #f59e0b; letter-spacing: 8px; font-family: monospace;">
            ${code}
          </span>
        </div>
      </div>
      ...
      <p style="color: #6b7280; font-size: 14px; line-height: 1.6; margin: 20px 0 0;">
        Questo codice scadrà tra <strong style="color: #9ca3af;">10 minuti</strong>.
      </p>
      ...
      <p style="color: #4b5563; font-size: 12px; margin: 0;">
        ...
        • Il nostro team non ti chiederà mai il codice via email o telefono<br>
        • Il codice è valido per un solo utilizzo
      </p>
    </div>
